...
@Composable
fun OnlineAuctionPlaceBid(navController: NavHostController) {

    val previewAuctionType: PreviewAuctionItemViewModel = hiltViewModel()
    val state = previewAuctionType.state

    val item = Session.currentAuctionItem.value ?: return
...
//    ChangeStatusBarColor(bgColor)
    ...
    }, containerColor = orangeBg) { paddingValues ->
        Column(
            modifier = Modifier
                .fillMaxSize()
            ...
            Box(modifier = Modifier.fillMaxWidth()) {
                Column(
                    ...
                    CoilImage(
                        imageModel = item.image, modifier = Modifier
                            .fillMaxWidth()
//                            .height(300.dp)
                    )
                    Column(
